		...
		<div class="ul_list ul_list-icon-ok">
			<ul>';
				foreach ($terms as $term) {
					$askme_private = get_term_meta($term->term_id,"vbegy_private",true);
					$askme_private = (isset($askme_private)?$askme_private:"");
					$term_list .= '<li><i class="'.($cats_tax == 'post' || $cats_tax == 'category'?'icon-file-alt':'icon-question-sign').'"></i>'.($askme_private == "on"?'<i class="icon-lock"></i>':'').'<a href="'.get_term_link($term->slug,($cats_tax == 'post' || $cats_tax == 'category'?'category':($cats_tax == 'product' || $cats_tax == 'product_cat'?'product_cat':ask_question_category))).'">'.$term->name.'<span> ( <span>'.$term->count." ";
					if ($term->count == 1) {
