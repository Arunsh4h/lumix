<?php if (is_page() && !is_page_template("template-search.php")) {
	$tag_sort  = askme_post_meta('vbegy_tag_sort','type=radio',$post->ID);
	$tag_style = askme_post_meta('vbegy_tag_style','type=radio',$post->ID);
	$tag_order = askme_post_meta('vbegy_tag_order','type=radio',$post->ID);
	$tags_tax  = askme_post_meta('vbegy_tags_tax','type=radio',$post->ID);
	$number    = askme_post_meta('vbegy_tags_per_page','type=text',$post->ID);
	...
	$tag_style = askme_options("tag_style_pages");
	...
	if (isset($not_get_result_page) && $not_get_result_page == true) {
		...
		$tags_tax = (isset($_GET["search_type"]) && $_GET["search_type"] != ""?esc_html($_GET["search_type"]):ask_question_tags);
	}
}
...
if ($tags_tax == 'post' || $tags_tax == 'post_tag') {
	...
	$tag_type = ask_question_tags;
	$post_type_tags = ask_questions_type;
}
...
if (isset($not_get_result_page) && $not_get_result_page == true) {
	...
	include locate_template("includes/tags-results.php");
}?>