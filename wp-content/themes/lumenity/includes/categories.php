<?php if (is_page() && !is_page_template("template-search.php")) {
	$cat_sort  = askme_post_meta('vbegy_cat_sort','type=radio',$post->ID);
	$cat_order = askme_post_meta('vbegy_cat_order','type=radio',$post->ID);
	$cats_tax  = askme_post_meta('vbegy_cats_tax','type=radio',$post->ID);
	$number    = askme_post_meta('vbegy_cats_per_page','type=text',$post->ID);
...
if ($cats_tax == 'post' || $cats_tax == 'category') {
	...
	$post_type_cats = ask_questions_type;
}
...
if (isset($not_get_result_page) && $not_get_result_page == true) {
	...
	include locate_template("includes/categories-results.php");
}?>