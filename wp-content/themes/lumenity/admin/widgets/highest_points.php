...
class Widget_Highest_Points extends WP_Widget
{
	...
	function __construct()
	{
		...
		parent::__construct('highest-points-widget', 'Lumeno - Highest Points', $widget_ops, $control_ops);
	}
	...
	function widget($args, $instance)
	{
		...
		$user_per_page = (isset($instance['user_per_page']) ? (int)$instance['user_per_page'] : '');
			...
			<div class="widget_highest_points">
				...
				if (isset($users) && is_array($users) && !empty($users)) {
					...
					foreach ($users as $key => $author) {
						...
						$your_avatar = get_user_meta($author->ID, askme_avatar_name(), true); ?>
						<li>
							<div class="author-img">
								<a href="<?php echo $user_profile_page ?>">
									<?php echo askme_user_avatar($your_avatar, 65, 65, $author->ID, $author->display_name); ?>
								</a>
							</div>
