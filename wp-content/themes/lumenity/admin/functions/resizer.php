 ...
 * $image = askme_resize( $thumb, '', 140, 110, true );
 * ?>
 *
 * @param int $attach_id
 ...
 * @return array
 */
if ( ! function_exists( 'askme_resize' ) ) :

	function askme_resize( $attach_id = null, $img_url = null, $width = 0, $height = 0, $crop = false , $gif = false , $retina = false ) {
		global  $blog_id;
		$img_url_o = $img_url;
		$divider = 1;
		if ($retina == true) {
		...
		if ( $image_src[1] > $width || $image_src[2] > $height ) {
			...
			if (is_writable($file_info['dirname'].'/')) {
				...
				if (isset($attach_id) && is_numeric($attach_id) && $attach_id > 0) {
					$get_post_meta = get_post_meta($attach_id,"askme_image_names",true);
					if ((is_array($get_post_meta) && empty($get_post_meta)) || !is_array($get_post_meta) || $get_post_meta == "") {
						$askme_image_names = array($dest_file);
					}else if (is_array($get_post_meta) && !in_array($dest_file,$get_post_meta)) {
						$askme_image_names = array_merge($get_post_meta,array($dest_file));
					}
					if (isset($askme_image_names)) {
						update_post_meta($attach_id,"askme_image_names",$askme_image_names);
					}
				}
...
function askme_image_crop_dimensions($default, $orig_w, $orig_h, $new_w, $new_h, $crop){
...
add_filter('image_resize_dimensions', 'askme_image_crop_dimensions', 10, 6);?>