<?php if ( class_exists( 'woocommerce' ) ) {
	...
	function wp_enqueue_woocommerce_style() {
		if (is_rtl()) {
			wp_register_style( 'woocommerce', get_template_directory_uri() . '/admin/woocommerce/woocommerce-rtl.css',array(),askme_theme_version );
			wp_register_style( 'woocommerce-responsive', get_template_directory_uri() . '/admin/woocommerce/woocommerce-media-rtl.css',array(),askme_theme_version );
		}else {
			wp_register_style( 'woocommerce', get_template_directory_uri() . '/admin/woocommerce/woocommerce.css',array(),askme_theme_version );
			wp_register_style( 'woocommerce-responsive', get_template_directory_uri() . '/admin/woocommerce/woocommerce-media.css',array(),askme_theme_version );
		}
		wp_register_script( 'woocommerce-js', get_template_directory_uri() . '/admin/woocommerce/woocommerce.js', array(), askme_theme_version);
	...
	function woocommerce_shop_thumbnail () {
		...
		$output .= askme_resize_img(230,280);
		$output .= woocommerce_add_to_cart_button();
		if(wc_get_product($product->get_id())->get_type() == 'simple') $output .= "<a href='$cart_url' class='woocommerce_cart_loading woocommerce_added_check'></a>";
		$output .= "</div>";
		echo $output;
	}
	//prducts per row
	...
	if (!function_exists('loop_columns')) {
		function woocommerce_loop_columns() {
			$cols = askme_options('shop_products_columns');
			if (empty($cols)) {
				$cols = 3;
			}
			return $cols; 
	...
	function woocommerce_add_to_cart_button() {
		...
		if (wc_get_product($product->get_id())->get_type() == 'bundle' ){
		...
		if (wc_get_product($product->get_id())->get_type() == 'variable' && empty($output)) {
			$output = "<a class='woocommerce_woo_bt button button' href='".get_permalink($product->get_id())."'>".__('Select options','vbegy')."</a>";
		}
		if ( class_exists( 'YITH_WCWL_UI' ) )  {
			//$output .= do_shortcode( "[yith_wcwl_add_to_wishlist]" );
		}
		if (wc_get_product($product->get_id())->get_type() == 'simple') {
			//$output .= "<br><a class='woocommerce_woo_bt show_details_button button' href='".get_permalink($product->get_id())."'>".__('Show details','vbegy')."</a>";
		}else {
			$btclass  = "single_bt";
		}
	...
	function vpanel_related_products() {
		...
		$related_number_s = askme_post_meta('vbegy_related_number_s','text',$post->ID);
		...
		if (($related_post == 1 && $related_post_s == "") || ($related_post == 1 && isset($custom_page_setting) && $custom_page_setting == 0) || ($related_post == 1 && isset($custom_page_setting) && $custom_page_setting == 1 && isset($related_post_s) && $related_post_s != 0) || (isset($custom_page_setting) && $custom_page_setting == 1 && isset($related_post_s) && $related_post_s == 1)) {
			if (isset($custom_page_setting) && $custom_page_setting == 1) {
				...
				if (isset($sidebar_post) && $sidebar_post == "full") {
					$related_number = askme_options('related_products_number_full');
					$related_number = (isset($related_number) && $related_number?$related_number:4);
				}else {
					$related_number = askme_options('related_products_number');
					$related_number = (isset($related_number) && $related_number?$related_number:3);
				}
			}
	...
	/* products per page */
	add_filter( 'loop_shop_per_page', 'askme_loop_shop_per_page');
	function askme_loop_shop_per_page () {
		$pcount = askme_options('woo_products_per_page');
		if ($pcount == '') {
			$pcount = 9;
		}
		return $pcount;
	...
	function product_share () {
		global $post;
		$post_views = askme_options("post_views");
		$post_share = askme_options("post_share");
		$post_share_s = askme_post_meta('vbegy_post_share_s','checkbox',$post->ID);
		$post_views_s = askme_post_meta('vbegy_post_views_s','checkbox',$post->ID);
		$custom_page_setting = askme_post_meta('vbegy_custom_page_setting','checkbox',$post->ID);
		if (($post_share == 1 && $post_share_s == "") || ($post_share == 1 && isset($custom_page_setting) && $custom_page_setting == 0) || ($post_share == 1 && isset($custom_page_setting) && $custom_page_setting == 1 && isset($post_share_s) && $post_share_s != 0) || (isset($custom_page_setting) && $custom_page_setting == 1 && isset($post_share_s) && $post_share_s == 1)) {
			...
			<div class="woo-share-social">
				<ul>
					<li class="social-facebook"><a href="https://www.facebook.com/sharer.php?u=<?php echo ($url);?>" target="_blank"><i class="icon-facebook"></i></a></li>
					<li class="social-twitter"><a href="http://twitter.com/share?text=<?php echo ($title);?>&amp;url=<?php echo ($url)?>" target="_blank"><i class="icon-twitter"></i></a></li>
					<li class="social-whatsapp"><a href="https://api.whatsapp.com/send?text=<?php echo ($title)?> - <?php echo ($url)?>" target="_blank"><i class="fa-brands fa-whatsapp"></i></a></li>
				</ul>
	...
	function woocommerce_bn_cart_button () {
		$nav_cart = askme_options('nav_cart');
		$nav_cart = 1;
		if ($nav_cart == 1) {
			...
			$in_woo = askme_options('nav_cart_in_woo');
