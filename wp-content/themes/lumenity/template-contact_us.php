...
get_header();
	if ( have_posts() ) : while ( have_posts() ) : the_post();
		$contact_map = askme_post_meta('vbegy_contact_map','textarea',$post->ID);
		$contact_form = askme_post_meta('vbegy_contact_form','text',$post->ID);
		$about_widget = askme_post_meta('vbegy_about_widget','checkbox',$post->ID);
		$vbegy_about_content = askme_post_meta('vbegy_about_content','textarea',$post->ID);
		$vbegy_address = askme_post_meta('vbegy_address','text',$post->ID);
		$vbegy_phone = askme_post_meta('vbegy_phone','text',$post->ID);
		$vbegy_email = askme_post_meta('vbegy_email','text',$post->ID);
		$vbegy_social = askme_post_meta('vbegy_social','checkbox',$post->ID);
		$vbegy_facebook = askme_post_meta('vbegy_facebook','text',$post->ID);
		$vbegy_tiktok = askme_post_meta('vbegy_tiktok','text',$post->ID);
		$vbegy_twitter = askme_post_meta('vbegy_twitter','text',$post->ID);
		$vbegy_youtube = askme_post_meta('vbegy_youtube','text',$post->ID);
		$vbegy_linkedin = askme_post_meta('vbegy_linkedin','text',$post->ID);
		$vbegy_instagram = askme_post_meta('vbegy_instagram','text',$post->ID);
		$vbegy_dribbble = askme_post_meta('vbegy_dribbble','text',$post->ID);
		$vbegy_pinterest = askme_post_meta('vbegy_pinterest','text',$post->ID);
		$vbegy_rss = askme_post_meta('vbegy_rss','checkbox',$post->ID);?>
									...
									if ($vbegy_social == 1) {?>
										<li>
											...
											<p>
												...
												<?php }
												if ($vbegy_tiktok != "") {?>
													<a href="<?php echo $vbegy_tiktok?>" original-title="<?php _e("TikTok","vbegy")?>" class="tooltip-n">
														<span class="icon_i">
															<span class="icon_square" icon_size="25" span_bg="#3b5997" span_hover="#2f3239">
																<i i_color="#FFF" class="fab fa-tiktok"></i>
															</span>
														</span>
													</a>
												<?php }
